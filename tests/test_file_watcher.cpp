...
TEST_CASE("FileWatcher reports NotFound when the watched directory is removed")
{
    ...
    // The in-flight read on the removed directory fails at the I/O level, so
    // no completion packet ever arrives; GetQueuedCompletionStatus itself
    // returns FALSE with the raw path-gone code (the CI's NTFS runner
    // reports ERROR_ACCESS_DENIED, the FILE_SHARE_DELETE handle outlives the
    // directory; ERROR_BROKEN_PIPE is the stale-handle variant). The failure
    // must map to NotFound, not Unknown, and the watch must then stay
    // quietly dead instead of going deaf or re-failing on every poll.

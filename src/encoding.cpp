...
namespace pjh::platform
{
    namespace
    {
        ...
        // East Asian Width (UAX #11) Wide (W) and Fullwidth (F) ranges, from
        // Unicode 15.1 EastAsianWidth.txt; only these occupy two columns.
        ...
        // Combining marks (Unicode Mn/Me) and format characters (Cf): zero
        // columns. Checked before kWideRanges so a combining mark that falls
        // inside an East Asian wide block (e.g. U+3099) stays zero-width.
        constexpr CodePointRange kZeroWidthRanges[] = {
            ...
            {0x0890, 0x0891},   {0x0898, 0x089F},   {0x08CA, 0x0902},   {0x093A, 0x093A},
            ...
            {0x13430, 0x13440}, {0x13447, 0x13455}, {0x16AF0, 0x16AF4}, {0x16B30, 0x16B36},
        ...
        [[nodiscard]] auto in_ranges(char32_t cp, const CodePointRange *ranges, std::size_t count)
            -> bool
        {
            std::size_t lo = 0;
            std::size_t hi = count;
        ...
        // Decodes the code point starting at utf8[offset]. `length` is always at
        // least 1 so callers can resynchronise; an invalid byte consumes exactly
        // one byte and keeps `value` equal to that byte.
        [[nodiscard]] auto decode_utf8(std::string_view utf8, std::size_t offset)
            -> DecodedCodePoint
        {
            ...
            result.value = static_cast<char32_t>(lead);

            std::size_t length = 0;
            char32_t cp = 0;
            if (lead < 0x80)
            {
                return DecodedCodePoint{static_cast<char32_t>(lead), 1, true};
            }
            ...
            {
                const unsigned char cont = bytes[offset + k];
                if ((cont & 0xC0) != 0x80)
                    return result;
        ...
        [[nodiscard]] auto code_point_width(char32_t cp) -> std::size_t
        {
            if (cp == U'\t')
                return 1;
            if (in_ranges(cp, kZeroWidthRanges, std::size(kZeroWidthRanges)))
                return 0;
            if (cp < 0x20 || (cp >= 0x7F && cp <= 0x9F))
                return 0;
            if (in_ranges(cp, kWideRanges, std::size(kWideRanges)))
                return 2;
            return 1;
        }
    }  // namespace
    ...
    // ── Display width ──────────────────────────────────────────────────────
    // Pure logic shared by every platform: the tables and classification below
    // are the only source of column widths, so all platforms agree.

    auto Encoding::display_width(std::string_view utf8) -> std::size_t
    {
        std::size_t width = 0;
        std::size_t offset = 0;
        while (offset < utf8.size())
        {
            const DecodedCodePoint decoded = decode_utf8(utf8, offset);
            width += decoded.valid ? code_point_width(decoded.value) : 1;
            offset += decoded.length;
        }
        return width;
    }

    auto Encoding::truncate_to_width(std::string_view utf8, std::size_t max_width)
        -> std::string_view
    {
        if (max_width == 0)
            return {};
        ...
        {
            const DecodedCodePoint decoded = decode_utf8(utf8, offset);
            const std::size_t cp_width = decoded.valid ? code_point_width(decoded.value) : 1;
            if (width + cp_width > max_width)
                break;
            width += cp_width;
            offset += decoded.length;
        }
        return utf8.substr(0, offset);
    }
